
#include "battery/constants.hpp"

namespace b {

    std::string Constants::BatteryIconBase64() noexcept {
        return "";
    }

    std::string Constants::DefaultApplicationName() noexcept {
        return "BatteryApp";
    }

    Platform Constants::Platform() noexcept {
#ifdef B_OS_WINDOWS
        return Platform::Windows;
#elif defined(B_OS_LINUX)
        return Platform::Linux;
#elif defined(B_OS_MACOS)
        return Platform::MacOS;
#elif defined(B_OS_WEB)
        return Platform::Web;
#else
        #error "Unknown platform"
#endif
    }

    Arch Constants::Architecture() noexcept {
#ifdef B_ARCH_64
        return Arch::x64;
#elif defined(B_ARCH_32)
        return Arch::x86;
#else
        #error "Invalid architecure configuration!"
#endif
    }

    double Constants::Pi() noexcept {
        return 3.141592653589793238462643383279502884197169399375105820974944592307816406286;
    }

#ifdef B_DEBUG
    bool Constants::MessageBoxOnExceptionDefault() noexcept { return true; }
    bool Constants::CatchCommonExceptionsDefault() noexcept { return true; }
#else
    bool Constants::MessageBoxOnExceptionDefault() { return false; }
    bool Constants::CatchCommonExceptionsDefault() { return true; }
#endif

    bool Constants::IsProductionMode() noexcept {
#ifdef B_PRODUCTION_MODE
        return true;
#else
        return false;
#endif
    }

    ImColor Constants::DefaultWindowBackgroundColor() noexcept {
        return { 20, 20, 20, 255 };
    }

    ImColor Constants::DefaultWindowTitlebarColor() noexcept {
        return { 25, 25, 25, 255 };
    }

    ImColor Constants::DefaultWindowFrameColor() noexcept {
        return { 56, 56, 56, 255 };
    }

    double Constants::DefaultFontSize() noexcept {
        return 18.f;
    }

    b::Vec2 Constants::DefaultWindowSize() noexcept {
        return { 1080, 720 };
    }

    std::string Constants::DefaultWindowTitle() noexcept {
        return "Battery Window";
    }

}  // namespace b
