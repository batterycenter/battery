...
// Here we use <codecvt> instead of the utf8cpp library, because the std is supposedly
// better in dealing with the wide strings on different platforms. (taking std::u16string
// from utf8cpp and converting to std::wstring might work on Windows but might cause
// issues on others.) For all other conversions, we use utf8cpp. <codecvt> is deprecated
// since C++17, but will not be removed any time soon as no alternative has been found
// yet. We will keep using it until it is removed or replaced by a better alternative.
                          ...
                          std::regex_constants::match_flag_type flags)
{
    auto result = std::regex_replace(
        widen(str), std::wregex(widen(pattern)), widen(replace), flags);
    return narrow(result);
}

std::vector<std::string> regex_match(const std::string& str,
                                     const std::string& pattern,
                                     std::regex_constants::match_flag_type flags)
{
...
std::string
replace(const std::string& str, const std::string& pattern, const std::string& value)
{
...
{
    return b::reverse(
        b::replace_first(b::reverse(str), b::reverse(pattern), b::reverse(value)));
}
...
std::string to_upper(const std::string& str)
{
    std::u32string result;
    for (const auto& chr : b::to_utf32(str)) {
        if (chr == U'ß') { // This special case is not handled well in the library (at
                           // least not how we want)
            result += U"SS";
            continue;
        }
