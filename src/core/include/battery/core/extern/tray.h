...
// === This file was largely modified to work with Battery.                              ===
...
static GtkMenuShell *_tray_menu(struct tray_menu *m) {
  ...
  for (; m != NULL && !m->text.empty(); m++) {
    GtkWidget *item;
    if (m->text == "-") {
      item = gtk_separator_menu_item_new();
    } else {
      if (m->submenu != NULL) {
        item = gtk_menu_item_new_with_label(m->text.c_str());
        gtk_menu_item_set_submenu(GTK_MENU_ITEM(item),
                                  GTK_WIDGET(_tray_menu(m->submenu)));
      } else {
        item = gtk_check_menu_item_new_with_label(m->text.c_str());
...
static int tray_init(struct tray *tray) {
  ...
  indicator = app_indicator_new(TRAY_APPINDICATOR_ID, tray->icon.c_str(),
...
static void tray_update(struct tray *tray) {
  app_indicator_set_icon(indicator, tray->icon.c_str());
    ...
    for (; m != NULL && !m->text.empty(); m++) {
      if (m->text.c_str() == "-") {
        ...
        objc_msgSend(menuItem, sel_registerName("initWithTitle:action:keyEquivalent:"),
                  objc_msgSend((id)objc_getClass("NSString"), sel_registerName("stringWithUTF8String:"), m->text.c_str()),
...
static void tray_update(struct tray *tray) {
  objc_msgSend(statusBarButton, sel_registerName("setImage:"),
    objc_msgSend((id)objc_getClass("NSImage"), sel_registerName("imageNamed:"),
      objc_msgSend((id)objc_getClass("NSString"), sel_registerName("stringWithUTF8String:"), tray->icon.c_str())));
...
static LRESULT CALLBACK _tray_wnd_proc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_TRAY_CALLBACK_MESSAGE:
    if (lparam == WM_LBUTTONUP || lparam == WM_RBUTTONUP) {
        struct tray* tray = reinterpret_cast<struct tray*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
        if (lparam == WM_LBUTTONUP) {
            if (tray->root_left_cb) {
                tray->root_left_cb();
            }
        }
        if (lparam == WM_RBUTTONUP) {
            if (tray->root_right_cb) {
                tray->root_right_cb();
            }
        }
        ...
        WORD cmd = TrackPopupMenu(hmenu, TPM_LEFTALIGN | TPM_RIGHTBUTTON |
                            ...
                            p.x, p.y, 0, hwnd, NULL);
        SendMessage(hwnd, WM_COMMAND, cmd, lparam);
        return 0;
    }
    break;
  case WM_COMMAND:
...
static HMENU _tray_menu(struct tray_menu *m, UINT *id) {
  ...
  for (; m != NULL && !m->text.empty(); m++, (*id)++) {
    if (m->text == "-") {
      ...
      item.wID = *id;
      auto os = battery::string::utf8_to_osstring(m->text);
      item.dwTypeData = (LPWSTR)os.c_str();
...
static void tray_update(struct tray *tray) {
  ...
  HICON icon;
  ExtractIconEx(battery::string::utf8_to_osstring(tray->icon).c_str(), 0, NULL, &icon, 1);
  if (nid.hIcon) {
