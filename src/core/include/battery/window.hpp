...
template <> struct glz::meta<b::WindowState> {
    using T = b::WindowState;
    static constexpr auto value
        = object("size", &T::size, "position", &T::position, "maximized", &T::maximized);
};
...
#ifndef B_OS_WEB
    std::optional<b::fs::path> lastWindowStateJsonFilePath
        = b::Folders::AppConfigDir() / "windowstate.json";
    std::optional<ImColor> win32CustomTitleBarColor
        = Constants::DefaultWindowTitlebarColor();
    std::optional<ImColor> win32CustomWindowFrameColor
        = Constants::DefaultWindowFrameColor();
...
public:
    ...
    template <typename T, typename... TArgs> auto attachEventHandler(TArgs&&... args)
    {
        return m_eventbus.attachEventHandler<T>(std::forward<TArgs>(args)...);
    }

    template <typename T, typename... TArgs> auto dispatchEvent(TArgs&&... args)
    {
