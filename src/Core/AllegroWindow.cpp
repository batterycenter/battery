...
namespace Battery {
	...
	void AllegroWindow::CenterOnPrimaryMonitor() {
		CHECK_ALLEGRO_INIT(); 
	...
	std::optional<Battery::Bitmap> AllegroWindow::GetClipboardImage() {
	...
	bool AllegroWindow::SetClipboardImage(const Battery::Bitmap& image) {
	...
	bool AllegroWindow::SetMouseCursor(int cursorID) {
		return al_set_system_mouse_cursor(allegroDisplayPointer, (ALLEGRO_SYSTEM_MOUSE_CURSOR)cursorID);
	}

	bool AllegroWindow::SetWindowExecutableIcon(int iconID) {
		CHECK_ALLEGRO_INIT();
